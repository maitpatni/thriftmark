...
const WidgetSocial: FC = () => {
  ...
  return (
    <div>
      <h4 className="mb-5 text-sm font-semibold text-heading md:text-base xl:text-lg 2xl:mb-6 3xl:mb-7 textWhiteOverride">
        {t(`footer:widget-title-social`)}
      </h4>
      <ul className="text-xs md:text-[13px] lg:text-sm text-body flex flex-col space-y-3 lg:space-y-3.5 textWhiteOverride">
        {socials?.map((social: Social, index: number) => (
          <li key={`widget-list--key${index}`} className="flex items-baseline">
            {social.icon && (
              <span className="ltr:mr-3 rtl:ml-3 relative top-0.5 lg:top-1 text-sm lg:text-base textWhiteOverride">
                {getIcon({
